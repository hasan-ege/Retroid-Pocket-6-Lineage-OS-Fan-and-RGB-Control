# Retroid Pocket 6 (RP6) — LineageOS Donanım Kontrolü: Teknik Referans

**Amaç:** Bu belge, RP6 üzerinde LineageOS 23.2 (Android 16 tabanlı) çalışan
bir cihazda, cihaz üzerinde bizzat test edilerek (aShellYou + adb root ile)
doğrulanmış donanım kontrol noktalarını belgeler. Bir sonraki geliştirme
oturumunda (insan ya da yapay zeka fark etmeksizin) sıfırdan reverse-engineering
yapmaya gerek kalmadan doğrudan buradan devam edilebilmesi amaçlanmıştır.
Her bulgunun yanında **doğrulama durumu** belirtilmiştir (✅ cihazda test
edildi / ⚠️ teorik, doğrulanmadı / ❌ denendi, çalışmadı).

---

## 0. Cihaz ve Ortam Bilgisi

| Alan | Değer |
|---|---|
| Cihaz | Retroid Pocket 6 (RP6), VARIANT: PRO |
| ROM | LineageOS 23.2 (Android 16 tabanlı) |
| Kernel | `5.15.288-g94a246947232` (cihazda `uname -r` ile teyit edildi) |
| Bootloader durumu | UNLOCKED, SECURE BOOT: NO |
| Root | Magisk (fastboot ile `init_boot` partisyonu patch'lenerek kuruldu — bu cihaz **GKI** mimarisi kullanıyor, `boot.img` değil `init_boot.img` patch'lenmeli) |
| Shell | **POSIX `sh` (toybox/mksh tabanlı), BASH DEĞİL** — `declare -a`, `< <(...)` process substitution gibi bash'e özgü sözdizimi ÇALIŞMIYOR. `eval "VAR_$i=$val"` deseniyle POSIX-uyumlu "sözde dizi" kullanılmalı. |
| Test aracı | aShellYou (Android üzerinde adb shell arayüzü) |

### 0.1 Kabuk/Ortam Tuzakları (önemli, tekrar düşmemek için)

- **`date +%s%N` (nanosaniye) bu cihazda güvenilmez/bozuk.** Ölçülen süre
  farkları tutarsız çıkabiliyor (bir testte "500 adım × ~88ms/adım" log'u
  basıldı ama gerçek toplam süre sadece 1.5 saniye çıktı — bu bir hataydı).
  **`sleep <ondalık>` ise doğru çalışıyor** (`sleep 0.5` gerçekte ~532ms
  sürdü, doğrulandı). Alt-saniye hassasiyetli zamanlama gerekiyorsa `sleep`'e
  güven, `date +%N`'e güvenme.
- **Sık `date`/subprocess çağrısı döngü içinde periyodik "takılma" yaratır**
  — bir animasyon döngüsünde her 10 adımda bir log basmak için `date`
  çağrıldığında, bu tam olarak "her 10 adımda bir duraklıyor" hissine yol
  açtı. Performans-kritik döngülerde log/ölçüm minimumda tutulmalı.
- **aShellYou'da salt-yazma komutları (`echo x > dosya`, `cp`) sık sık
  "Stream closed" hatası veriyor**, bu genelde işlemin başarısız olduğu
  anlamına gelmez — çıktı beklenen bir şey olmadığı için aShellYou bağlantıyı
  erken kesiyor gibi görünüyor. **Güvenilir doğrulama için yazma+okumayı tek
  satırda birleştir:** `echo X > dosya; cat dosya`.
- **Büyük dosya transferi (APK çekme vb.) için aShellYou'nun `cp`'si
  güvenilmez** (root ile bile "Stream closed" ile başarısız oldu). **MTP
  üzerinden (bilgisayara USB ile bağlayıp dosya gezgininden) çekmek**
  belirgin şekilde daha güvenilir sonuç verdi.
- **`toybox sleep` ondalık destekliyor, ama bazı komutlar (`declare`,
  process substitution) desteklemiyor** — her yeni script'te önce POSIX
  uyumluluğunu göz önünde bulundur.
- **Root'suz/Shizuku seviyesinde (shell/uid 2000) erişim test edildi ve
  YETERSİZ çıktı (✅ doğrulandı):** `cur_state` dosyası `-rw-r--r--`
  (root:root) — okunabiliyor ama yazılamıyor. LED dosyaları
  (`multi_intensity`, `brightness`) ise root'suzken `ls` ile bile
  "Permission denied" veriyor (okunamıyor bile). **Sonuç: bu cihazda fan
  ve RGB kontrolü için gerçek root (`su`) şart, Shizuku/adb-shell seviyesi
  yeterli değil.** Root'suz kullanıcılara ulaşmanın tek yolu, uygulamanın
  priv-app/sistem uygulaması olarak (özel build ya da resmi upstream katkı
  yoluyla) imzalanması — bkz. Implementation Plan Bölüm 26 (Dağıtım
  Stratejisi, planlandı).

---

## 1. Fan Kontrolü

### 1.1 Kontrol Noktası (✅ doğrulandı)

```
/sys/class/thermal/cooling_device34/cur_state   (yazılabilir, 0-8 tam sayı)
/sys/class/thermal/cooling_device34/max_state   (salt okunur, = 8)
```
`cooling_device34`'ün `type` dosyası `pwm-fan` döndürür — standart Linux
thermal framework cooling device'ı. **UYARI: bu index numarası (`34`)
kernel güncellemeleriyle değişebilir, kod her zaman `type == "pwm-fan"`
olan cooling device'ı dinamik arayarak bulmalı, sabit path'e güvenmemeli.**

### 1.2 Kalıcı Yazma Sorunu ve Çözümü (✅ doğrulandı)

Kernel'in `step_wise` termal governor'ı, bu cooling device'ı **~1 saniye
içinde otomatik olarak eski değerine geri döndürüyor** — tek seferlik
`echo 8 > cur_state` yazmak fiziksel olarak fanı hızlandırmaya yetmiyor
(test: tek yazma sesli/hissedilir bir değişiklik yaratmadı; 10 saniyelik
sürekli-yeniden-yazma döngüsü ise net bir hızlanma yarattı, RPM'e benzer
şekilde doğrulandı).

**Çözüm: periyodik yeniden yazma döngüsü** (300ms önerilir, cihazda daha
sık — 100-300ms aralığı — da test edildi ve çalıştı):
```bash
while true; do echo <hedef> > /sys/class/thermal/cooling_device34/cur_state; sleep 0.3; done
```

**Neden `user_space` governor'a geçip döngüden kurtulamıyoruz (✅ araştırıldı, güvenli değil):**
`cooling_device34`, şu 5 thermal zone'a bağlı: `thermal_zone14`(cpu-1-9),
`15`(cpu-1-10), `33`(gpuss-0), `34`(gpuss-1), `53`(usb-therm). Bunlardan
14/15/33/34, **fan dışında** kritik CPU/GPU korumalarını da yönetiyor:
- zone14/15 → aynı zamanda `cpu-hotplug7` ve `thermal-pause-80` cooling
  device'larını da tetikliyor (çekirdek açma/kapama ve zorla durdurma).
- zone33/34 → aynı zamanda `devfreq-3d00000.qcom,kgsl-3d0` (GPU frekans
  ölçekleme koruması) cooling device'ını da tetikliyor.
Sadece `zone53` (usb-therm) "temiz" (yalnız fan) ama tek başına
`user_space`'e almak yetersiz. **Sonuç: döngüyle üzerine yazma, kernel'in
güvenlik mantığını bozmayan doğru yaklaşım — bir "hack" değil.**

### 1.3 RPM / Gerçek Devir Verisi (❌ mevcut değil, doğrulandı)

- `hwmon0` adlı bir cihaz var (`name` = `pwmfan`, PMIC `pm8550` PWM çıkışı,
  devicetree yolu `.../qcom,pm8550@1/pwm-fan@0`), ama içinde sadece `pwm1`
  var — **`fan1_input` (RPM) yok.**
- `max31760_fan.ko` modülü yüklü (`i2c:max31760` sürücüsüyle eşleşmiş
  görünüyor) ama **hiçbir I2C cihaz adresine bağlı değil** — ölü/kullanılmayan
  modül. Gerçek tacometre okuma (stock ROM'daki gibi) LineageOS kernel'inde
  **kasıtlı ya da kazara portlanmamış.**
- Stock ROM'da (`gpio5_pwm2/speed`) RPM okunabiliyordu (test: kademe 8'de
  ~6600 RPM, kademe 0'da ~4200 RPM), ama bu Retroid'in kendi custom
  `gpio5-pwm` sürücüsüne özeldi, LineageOS'ta bu sürücü/sysfs yolu **hiç
  yok** (`/sys/class/gpio5_pwm2/` LineageOS'ta mevcut değil).
- **Sonuç: UI'da RPM yerine `cur_state/max_state` (örn. "6/8") gösterilmeli.**

### 1.4 Kesirli Hedef (Dithering) (✅ doğrulandı)

`cur_state` tam sayı zorunlu. `4.3` gibi bir hedef, zamanın `%30`'unda `5`,
`%70`'inde `4` yazarak (PWM dithering) simüle edilebilir — cihazda 40 adımlık
bir dithering testi çalıştırıldı, davranış beklenen gibiydi.

### 1.5 Stock ROM Referansı (yalnızca bilgi amaçlı, LineageOS'ta YOK)

Stock ROM'da fan, `com.rp.settings` (RPSettings.apk) paketi içindeki
`SettingsController` sistem servisi (`com.ro.settings.IExternalControlManager`
arayüzü) üzerinden, `/sys/class/gpio5_pwm2/{duty,speed,state,period,thermal}`
sysfs dosyalarına yazılarak kontrol ediliyordu. `com.rp.gameassistant`
(GameAssistant.apk) da aynı servise `com.ro.settings.IFanControlProvider`
adlı bir binder arayüzüyle bağlanan bir istemciydi. **Bu paketler LineageOS'a
taşınmadı çünkü fan kontrolü zaten doğrudan thermal framework üzerinden
(Bölüm 1.1-1.2) mümkün olduğu keşfedildi — taşımaya gerek kalmadı.**

---

## 2. Joystick RGB LED Kontrolü

### 2.1 Kontrol Noktası (✅ doğrulandı)

Standart Linux **multicolor LED class** (`CONFIG_LEDS_CLASS_MULTICOLOR`):
```
/sys/class/leds/left:stick:{0,1,2,3}/{multi_intensity, brightness, max_brightness}
/sys/class/leds/right:stick:{0,1,2,3}/{multi_intensity, brightness, max_brightness}
```
- `multi_intensity`: `"<R> <G> <B>"` formatında, her kanal 0-255, `multi_index`
  dosyası sırayı `red green blue` olarak teyit ediyor.
- `brightness`: 0-255, genel parlaklık/commit.
- **Fan'ın aksine bu, bir kernel governor'ı tarafından ezilmiyor — tek
  seferlik yazma kalıcı kalıyor.** Döngüye gerek yok (yalnızca animasyon
  isteniyorsa döngü kurulur, zorunluluktan değil).
...
### 2.2 Fiziksel Konum Haritası (✅ cihazda görsel olarak doğrulandı)

**Sol joystick** (düz sıra):
| Index | Konum |
|---|---|
| 0 | Üst-Sol |
| 1 | Alt-Sol |
...
**Sağ joystick** (⚠️ sol ile SİMETRİK DEĞİL, noktasal/180° döndürülmüş):
| Index | Konum |
|---|---|
| 0 | Alt-Sağ |
| 1 | Üst-Sağ |
...
**Kritik hata payı:** "Aynı index = aynı köşe" varsayımı YANLIŞ. Kod, iki
joystick için ayrı harita tablosu kullanmalı.

### 2.3 Döner Gökkuşağı Animasyonu (✅ doğrulandı, yön dahil)

**Teknik:** Her köşeye saat yönünde sabit faz atanır: Üst-Sol=0°,
Üst-Sağ=90°, Alt-Sağ=180°, Alt-Sol=270°. `cornerHue = (taban_hue + faz) %
360`. **Taban hue zamanla ARTTIRILIRSA desen saatin TERSİ yönde akar;
AZALTILIRSA (mod 360 ile sarmalanarak) SAAT YÖNÜNDE akar** (bu matematiksel
sonuç "moving rainbow" tekniğinin standart bir özelliği, cihazda da bu yönde
doğrulandı).

**Performans (✅ ölçüldü):** 8 LED'in tamamını yazmak (I2C üzerinden,
`2-003c`/`3-003c` yollarında duran donanım) ~15-29ms sürüyor — darboğaz
burası değil. `brightness`'ı döngü dışında BİR KERE ayarlayıp döngü içinde
yalnızca `multi_intensity` yazmak yazma sayısını yarıya indirir.

**HSV→RGB dönüşümü** (awk ile, cihazda çalışan formül):
```awk
c=1; x=c*(1-((h/60)%2>1?2-(h/60)%2:(h/60)%2));
...
## 3. LineageOS Ayarlar Uygulamasına Enjeksiyon (Menü Ekleme)

### 3.1 Mekanizma (✅ bu ROM'da gerçek, çalışan bir örnekle doğrulandı)

AOSP Settings, resmi olarak şu action'a sahip aktiviteleri kendi listesine
otomatik ekler — Settings kaynağı değiştirilmeden/yeniden derlenmeden:
```
Action: com.android.settings.action.IA_SETTINGS
Category: android.intent.category.DEFAULT
Meta-data: com.android.settings.category = com.android.settings.category.ia.homepage (veya .system/.device/.display)
Meta-data: com.android.settings.summary = "<açıklama>"
```

**Cihazda bulunan gerçek örnek:** `org.lineageos.settings.ayn` paketi
(`.AynSettingsActivity`), bu tam olarak yukarıdaki mekanizmayı kullanıyor
ve Ayarlar > (bir kategori altında) "Handheld"/"Gamepad" gibi bir girdi
olarak görünüyor. Bu, cihazda **çalıştığı kanıtlanmış** bir referans.

### 3.2 `org.lineageos.settings.ayn` Paketi — Detaylı Bulgular

Bu paket, gerçek RP6 donanımı için değil **AYN Odin** adlı başka bir el
konsolu için yazılmış bir LineageOS "device parts" modülü, RP6'da da
(donanım yeterince benzer olduğu için mi, yoksa geçici çözüm olarak mı
belirsiz) kullanılıyor.

- **İzin:** `android.permission.SET_KEYBOARD_LAYOUT` verilmiş —
  **signature/privileged seviyeli bir izin**, normal `pm grant` ile (root
  ile bile) sideload edilmiş bir uygulamaya verilemeyebilir; muhtemelen
  paketin priv-app olarak whitelist'te olmasından kaynaklanıyor. Bu, "Layout:
  Xbox/Nintendo" ayarının **`InputManager.setCurrentKeyboardLayoutForInputDevice()`**
  (hidden/system API) üzerinden çalıştığını gösteriyor.
- **`/data/data/org.lineageos.settings.ayn` tamamen boş** — SharedPreferences/
  veritabanı kullanmıyor, ayar bilgisi **sistem seviyesinde** (`InputManager`'ın
  kendi kalıcı deposunda) tutuluyor, uygulamanın kendi verisinde değil.
- **Manifest'te tek aktivite var** (`AynSettingsActivity`) — Layout, Trigger
  mode, M1/M2, Joystick calibration muhtemelen bu tek ekranın içinde
  (fragment/alt-navigasyon olarak) toplanmış, ayrı ayrı export edilmiş
  aktiviteler değil (⚠️ `exported` durumu ve intent-extra ile alt ekranlara
  atlanıp atlanamayacağı henüz APK decompile edilerek doğrulanmadı).

### 3.3 Bu Paketten Gözlemlenen Ayar Ekranı İçeriği (✅ ekran görüntüsüyle doğrulandı)

"Gamepad" ekranında şu seçenekler var:
- **Layout:** Nintendo / Xbox (A/B/X/Y fiziksel-mantıksal eşleme değişimi)
- **Trigger mode:** Both (vs. muhtemelen dijital/analog gibi diğer seçenekler)
- **M1 / M2:** varsayılan `None` — makro atama slotları
- **Joystick calibration** → **"Calibrate analog sticks"** alt-ekranı/eylemi

### 3.4 Strateji (henüz uygulanmadı, plan aşamasında — bkz. eklenen Implementation Plan Bölüm 22-24)

"En kötü ihtimalde" (kendi remap/kalibrasyon mantığımızı yazmak yerine)
kendi uygulamamızdan bu ekranı/eylemi tetiklemek — `SET_KEYBOARD_LAYOUT`
iznini alamazsak bile, `AynSettingsActivity`'yi (exported ise) `Intent` ile
başlatmak yeterli olabilir.

---

## 4. Sensör Yönelim Hatası (Gyro/Ekran 90° Kaymış) — ÇÖZÜLDÜ

### 4.1 Sorun (✅ resmi, bilinen bir LineageOS bug'ı — GitHub Issue #254, LineageOS/issues, hâlâ "Open")

RP6'da ekran otomatik döndürme (ve dolayısıyla ham gyro/accelerometer verisi,
`qmi8658` çipi) **90° saat yönünde kaymış** geliyor. Bu, `rocknix_abl` gibi
üçüncü parti bootloader/dual-boot değişikliklerinden **bağımsız**, saf bir
LineageOS/RP6 vendor konfigürasyon sorunu (issue'da bu ayrıca teyit edildi).

### 4.2 Resmi Önerilen Düzeltme (⚠️ TEK BAŞINA YETERSİZ, kısmi çözüm)

Maintainer (`AnierinBliss`) önerisi:
```bash
adb root
adb remount
adb shell sed -i 's/version=2/version=3/g' /vendor/etc/sensors/sns_reg_config
adb reboot
```
Mantık: device tree'de (`android_device_retroidpocket_RP6`,
`extract-files.py#L22`) yönelim matrisi zaten düzeltilmiş durumda, ama bu
sadece **sensör registry'sinin config `version` numarası arttığında yeniden
inşa edilmesiyle** etkinleşiyor. **Bazı cihazlarda (raporlayan kullanıcı ve
en az bir başka kullanıcı `hasan-ege` dahil) `version=3`'e geçmesine rağmen
sorun devam etti** — yani registry'nin version değişikliğine rağmen fiilen
yeniden inşa edilmediği ayrı bir sorun var.

### 4.3 Gerçekten İşe Yarayan Tam Çözüm (✅ bu cihazda doğrulandı — bizzat test edildi ve düzeldi)

```bash
adb root
adb shell rm -rf /mnt/vendor/persist/sensors/registry/*
adb reboot
```
Registry'yi **tamamen silip sıfırdan inşa etmeye zorlamak** kesin çözüm oldu
(4.2'deki `sed` adımını da önden uygulamış olmak zorunlu değil ama zarar
vermiyor, birlikte uygulanabilir). Bu, `AnierinBliss`'in "B" ihtimali
(*"Sensor registry rebuild wasn't triggered for some reason"*) ile birebir
örtüşüyor.

### 4.4 OTA Güncellemesi Sonrası Beklenti (⚠️ teorik, doğrulanmadı)

`/mnt/vendor/persist/` genel olarak OTA güncellemelerinden etkilenmeyen
kalıcı bir partisyon olduğu için, bu düzeltmenin bir sonraki OTA'dan sonra
da kalıcı kalması **beklenir**, ama kesin değildir — ilk OTA sonrası
`dumpsys sensorservice` ile (ya da fiziksel döndürme testiyle) tekrar
doğrulanmalı. (`rocknix_abl` kullanan kullanıcılar için ayrıca: OTA,
`abl` partisyonunu resmi LineageOS bootloader'ıyla değiştireceği için
dual-boot köprüsü ayrıca yeniden kurulmalı — ama bu, sensör düzeltmesinden
bağımsız bir konu.)

---

## 5. Cihazda Bulunan İlgili Paketler (referans)

| Paket | Rol | LineageOS'ta var mı |
|---|---|---|
| `com.rp.gameassistant` | Stock fan/performans UI + oyun içi overlay istemcisi | ✅ kurulu ama muhtemelen fan servisine bağlanamıyor (servis yok) |
| `com.rp.settings` (RPSettings) | Stock fan/ayar servis sağlayıcısı, `FanTempControlCurveConfigActivity` içeriyor | ✅ kurulu, aynı durum |
| `com.rp.mapping` (RsMapping) | Buton eşleme | ✅ kurulu, işlevi netleştirilmedi |
| `com.rp.factorytest` | Fabrika/QC donanım test uygulaması — **tüm sensör/buton/ekran testleri için kullanışlı olabilir, henüz açılıp incelenmedi** | ✅ kurulu, keşfedilmedi |
| `org.lineageos.settings.ayn` | Gerçek, resmi LineageOS "device parts" ayarlar modülü (Layout/Trigger/M1-M2/Kalibrasyon) | ✅ kurulu ve aktif kullanılıyor |

---

## 6. Genel Ders / Metodoloji Notu (bir sonraki AI/geliştirici için)

Bu cihazda donanım kontrolü ararken izlenen ve işe yarayan genel yöntem:
1. `pm list packages` ile ilgili olabilecek paketleri tara (`com.rp.*`,
   `org.lineageos.settings.*` gibi cihaza özel isimler ara).
2. `/sys/class/`, `/sys/firmware/devicetree/base/soc/`, `/proc/modules`
   altında donanım adına yakın anahtar kelimelerle (`find -iname "*x*"`)
   sistematik tara.
3. Bulunan sysfs dosyalarını **önce oku** (`cat`), sonra **yaz+hemen oku**
   ile test et (write-only komutlar aShellYou'da yanıltıcı hata verebiliyor).
4. Bir değer "geri sıçrıyorsa" (kernel governor'ı eziyor), önce bunun
   *hangi* thermal zone/governor'dan geldiğini bul, o zone'un **başka neyi
   de kontrol ettiğini** mutlaka kontrol et (`user_space`'e almadan önce).
   Güvenli değilse döngüyle yeniden yazma tercih edilmeli.
5. Resmi GitHub issue tracker'ını (`LineageOS/issues`) cihaz kod adıyla
   (`RP6`) ara — bilinen sorunlar ve (bazen yarım/hatalı da olsa) önerilen
   çözümler çoğunlukla zaten belgelenmiş oluyor.
-e 

---

# EK: TAM IMPLEMENTATION PLAN

*(Aşağıdaki bölüm, yukarıdaki teknik bulgulara dayanan tam Android uygulama geliştirme planıdır — 25 bölüm, fan/RGB/CPU/pil/overlay/settings-injection dahil tüm özellikleri kapsar.)*
